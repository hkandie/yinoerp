...
namespace vacarian\controllers;
...
//------------------------------------------------------------------------------------------------//
    ...
    public function actionIndex() {
        $model = new \yii\base\DynamicModel([
            'subject', 'price', 'level', 'location'
        ]);
        Yii::$app->session->set('userid', array(0));
        $model->addRule(['subject', 'price', 'level'], 'required');

        if ($model->load(Yii::$app->request->post())) {
            $searchModel = new \common\modules\user\models\ProfileSearch;
            $queryParams = array_merge(array(), Yii::$app->request->getQueryParams());
            $queryParams["ProfileSearch"]["note"] = $model->subject;
            $queryParams["ProfileSearch"]["min_hire_cost"] = $model->price;
            $queryParams["ProfileSearch"]["level"] = $model->level;
            $queryParams["ProfileSearch"]["location"] = $model->location;
            //$queryParams["CotizacionSearch"]["status"] = "DRAFT";
            $dataProvider = $searchModel->search($queryParams);
            Yii::$app->session->set('search',$queryParams);
    ...
    public function actionContact() {
        ...
        $orders = new \common\modules\order\models\Orders;
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            $orders->name = $model->first_name . " " . $model->last_name;
            $orders->user_id = 0;
            $orders->comments = json_encode($session->get('userid'))
                    ."|".json_encode($session->get('search'))
                    ."|".json_encode($model);
            $orders->txdate = date("Y-m-d");
            if ($session->get('userid') != null) {
                $orders->save(false);
            }
            $dataProvider = new \yii\data\ActiveDataProvider([
                'query' => \common\modules\user\models\Profile::find()->where(['user_id'=>$session->get('userid')]),
                
            ]);
            return $this->renderPartial('search', [
                        'dataProvider' => $dataProvider,'saved'=>true
            ]);
        }
