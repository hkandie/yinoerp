...
class m150214_044831_init_user extends Migration
{
    public function safeUp()
    {
        ...
        $this->createIndex('{{%user_email}}', '{{%_users}}', 'email', true);
        $this->createIndex('{{%user_username}}', '{{%_users}}', 'username', true);
        $this->createIndex('{{%user_key_key}}', '{{%_user_key}}', 'key_value', true);
        $this->createIndex('{{%user_auth_provider_id}}', '{{%_user_auth}}', 'provider_id', false);

        // add foreign keys for data integrity
        $this->addForeignKey('{{%user_role_id}}', '{{%_users}}', 'role_id', '{{%_role}}', 'id');
        $this->addForeignKey('{{%profile_user_id}}', '{{%_profile}}', 'user_id', '{{%_users}}', 'user_id');
        $this->addForeignKey('{{%user_key_user_id}}', '{{%_user_key}}', 'user_id', '{{%_users}}', 'user_id');
        $this->addForeignKey('{{%user_auth_user_id}}', '{{%_user_auth}}', 'user_id', '{{%_users}}', 'user_id');

        // insert role data
        $columns = ['name', 'can_admin', 'create_time'];
        $this->batchInsert('{{%_role}}', $columns, [
        ...
        $this->batchInsert('{{%_users}}', $columns, [
            [
                Role::ROLE_ADMIN,
                'zeddarn@gmail.com',
                'neo',
        ...
        $this->batchInsert('{{%_profile}}', $columns, [
    ...
    public function safeDown()
    {
        ...
        $this->dropTable('{{%_user_auth}}');
        $this->dropTable('{{%_profile}}');
        $this->dropTable('{{%_user_key}}');
        $this->dropTable('{{%_users}}');
        $this->dropTable('{{%_role}}');
    }
}
